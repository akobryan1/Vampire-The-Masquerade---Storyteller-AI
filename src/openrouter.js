...
export function buildSystemPrompt({ guardrails, city, hooks, subplotSeeds, mainPlotSeeds, npcSeeds = [], npcReferenceSheets = '', npcConversionWorkflow = '', chronicle, character, characterSummaryMode = 'full' }) {
  const hookText = hooks.length
    ? hooks.map((hook) => `- ${hook.title}: ${hook.summary}`).join('\n')
    : '- No active plot hooks selected.';
  const subplotText = Array.isArray(subplotSeeds) && subplotSeeds.length
    ? subplotSeeds.map((seed) => `- ${seed.title}: ${seed.summary}`).join('\n')
    : '- No subplot seeds provided.';
  const mainPlotText = Array.isArray(mainPlotSeeds) && mainPlotSeeds.length
    ? mainPlotSeeds.map((seed) => `- ${seed.title}: ${seed.summary}`).join('\n')
    : '- No main-plot seeds provided.';
  const npcSeedText = Array.isArray(npcSeeds) && npcSeeds.length
    ? npcSeeds.map((seed) => `- ${seed.name} (${seed.clan}, ${seed.role}): ${seed.summary}`).join('\n')
